...
class TweetApiTests(TestCase):
    ...
    def setUp(self):
        ...
        self.tweets2 = [
            self.create_tweet(self.dongxie)
            for i in range(2)
        ]
    ...
    def test_list_api(self):
        ...
        response = self.anonymous_client.get(TWEET_LIST_API, {'user_id': self.dongxie.id})
    ...
    def test_create_api(self):
        ...
        # 必须带 content
        response = self.linghu_client.post(TWEET_CREATE_API)
        self.assertEqual(response.status_code, 400)
        # content 不能太短
        response = self.linghu_client.post(TWEET_CREATE_API, {'content': '1'})
        self.assertEqual(response.status_code, 400)
        # content 不能太长
        response = self.linghu_client.post(TWEET_CREATE_API, {
            'content': '0' * 141
    ...
    def test_retrieve(self):
        ...
        self.create_comment(self.dongxie, tweet, 'holly s***')
        self.create_comment(self.linghu, tweet, 'hmm...')
        response = self.anonymous_client.get(url)
        self.assertEqual(len(response.data['comments']), 2)

        # tweet 里包含用户的头像和昵称
        profile = self.linghu.profile
    ...
    def test_create_with_files(self):
        ...
        response = self.linghu_client.get(retrieve_url)
    ...
    def test_pagination(self):
        ...
        for i in range(page_size * 2 - len(self.tweets1)):
            self.tweets1.append(self.create_tweet(self.linghu, 'tweet{}'.format(i)))

        tweets = self.tweets1[::-1]

        # pull the first page
        response = self.linghu_client.get(TWEET_LIST_API, {'user_id': self.linghu.id})
        ...
        response = self.linghu_client.get(TWEET_LIST_API, {
        ...
        new_tweet = self.create_tweet(self.linghu, 'a new tweet comes in')
